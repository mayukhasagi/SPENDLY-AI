...
def identify_anomalies(df):
    ...
    monthly_spend["rolling_mean"] = monthly_spend.groupby("exp_type")["amount_raw"].transform(_rolling_mean_prev)
    monthly_spend["rolling_std"] = monthly_spend.groupby("exp_type")["amount_raw"].transform(_rolling_std_prev)

    monthly_spend["z_score"] = np.nan
    valid = monthly_spend["rolling_std"].notna() & (monthly_spend["rolling_std"] > 0) & monthly_spend["rolling_mean"].notna()
    monthly_spend.loc[valid, "z_score"] = (
        (monthly_spend.loc[valid, "amount_raw"] - monthly_spend.loc[valid, "rolling_mean"]) / monthly_spend.loc[valid, "rolling_std"]
    )
    ...
    iqr = q3 - q1
    monthly_spend["is_outlier_iqr"] = (monthly_spend["amount_raw"] > (q3 + 1.5 * iqr)) | (monthly_spend["amount_raw"] < (q1 - 1.5 * iqr))
    ...
    if not anomalies.empty:
        anomalies["explanation"] = anomalies.apply(get_explanation, axis=1)
    else:
        anomalies["explanation"] = ""

    final_anomalies = anomalies[["month", "exp_type", "amount_raw", "z_score", "explanation", "method"]]
    os.makedirs("outputs", exist_ok=True)
    final_anomalies.to_csv("outputs/anomaly_df.csv", index=False)
...
def perform_clustering(df):
    ...
    monthly_spend = df.groupby(["month", "exp_type"])["amount_raw"].sum().reset_index()
    pivot_df = monthly_spend.pivot_table(index="month", columns="exp_type", values="amount_raw", aggfunc="sum", fill_value=0)
    ...
    for cid in sorted(cluster_means.index.tolist()):
        ...
        if overall_mean_total > 0 and total < 0.6 * overall_mean_total:
            labels[cid] = "Low activity month"
        elif total > 1.4 * overall_mean_total:
            labels[cid] = f"High spend month (especially {top_cat})"
        else:
            labels[cid] = f"{top_cat}-heavy month"
    ...
    os.makedirs("outputs", exist_ok=True)
    cluster_df.to_csv("outputs/cluster_labels.csv", index=False)
...
def main():
    ...
    print("Pattern/anomaly tasks completely finished.")
